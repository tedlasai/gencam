...
def get_3d_sincos_pos_embed(
    ...
    if not use_temporal:
        pos_embed_temporal = pos_embed_temporal * 0.
...
class CAPPatchEmbed(nn.Module):
    def __init__(
        ...
        if use_positional_embeddings or use_learned_positional_embeddings:
            ...
            pos_embedding, ref_embedding, text_embedding = self._get_positional_embeddings(
                sample_height, sample_width, sample_frames
            )
            self.register_buffer("pos_embedding", pos_embedding, persistent=persistent)
            self.register_buffer("ref_embedding", ref_embedding, persistent=persistent)
            self.register_buffer("text_embedding", text_embedding, persistent=persistent)
    ...
    def _get_positional_embeddings(
        ...
        text_embedding = pos_embedding.new_zeros(
            1, self.max_text_seq_length, self.embed_dim, requires_grad=False
        )
        ref_embedding = get_3d_sincos_pos_embed(
            self.embed_dim,
            (post_patch_width, post_patch_height),
            1,
            self.spatial_interpolation_scale,
        ...
        # print("dims", post_patch_width, post_patch_height, self.embed_dim, self.spatial_interpolation_scale, self.temporal_interpolation_scale, post_time_compression_frames)
        # print("pos_embedding", pos_embedding.shape)
        # print("ref_embedding", ref_embedding.shape)
        # exit()
        # joint_pos_embedding.data[:, self.max_text_seq_length :].copy_(pos_embedding)

        return pos_embedding, ref_embedding, text_embedding
    ...
    def forward(
        ...
        if self.patch_size_t is None:
            for img_embed, img_cond, seq_info in zip(image_embeds, image_conds, sequence_infos):
                ...
                if seq_info is None:
                    pos_embed = self.ref_embedding[None]
                else:
                    pos_embed = self.pos_embedding[None, seq_info]
