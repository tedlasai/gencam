...
def write_video(tensor, filename='output.mp4', fps=20):
    # tensor: (1, num_gts, 3, h, w)
    import cv2
    tensor = tensor.squeeze(0)  # (num_gts, 3, h, w)
    num_frames, _, h, w = tensor.shape
    from videoio import videosave
    print(tensor.shape)
    tensor = tensor.permute(0, 2, 3, 1).cpu().numpy()  # (num_gts, h, w, 3)
    tensor = tensor.clip(0, 255).astype(np.uint8)  # (num_gts, h, w, 3)
    videosave(filename, tensor,fps=fps)
...
def evaluate(d_model, p_model, valid_loader, local_rank):
    ...
    for i, tensor in enumerate(tqdm(valid_loader, total=len(valid_loader))):
        ...
        for _ in range(num_sampling):
            p_tensor = {}
            print("P tensor inp shape", tensor['inp'].shape)
            ...
            tensor['trend'] = p_tensor['trend']# out_tensor['pred_trends'].to(device) - GIVE IT THE GT TREND
            tensor['trend'] = F.interpolate(tensor['trend'], size=(256, 256))[None]
            ...
            import os
            os.makedirs('/home/tedlasai/genCamera/Animation-from-Blur-main/videos_out', exist_ok=True)
            print("Max pred imgs", pred_imgs.max())
            print("Min pred imgs", pred_imgs.min())
            write_video(torch.clamp(pred_imgs.squeeze(0),0,255), filename=f'/home/tedlasai/genCamera/Animation-from-Blur-main/videos_out/{i}.mp4', fps=10)
