...
def evaluate(d_model, p_model, valid_loader, local_rank):
    ...
    for i, tensor in enumerate(tqdm(valid_loader, total=len(valid_loader))):
        ...
        print("B, num_gts, c, h, w", b, num_gts, c, h, w)
        ...
        for _ in range(num_sampling):
            p_tensor = {}
            p_tensor['inp'] = F.interpolate(tensor['inp'], size=(3, 512, 512))
            p_tensor['trend'] = F.interpolate(tensor['trend'], size=(2, 512, 512))
            p_tensor['video'] = ''
            ...
            import os
            os.makedirs('/home/tedlasai/genCamera/Animation-from-Blur-main/baist_videos_out', exist_ok=True)
